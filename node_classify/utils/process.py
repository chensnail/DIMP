...
def load_data(dataset_str): # {'pubmed', 'citeseer', 'cora'}
    ...
    return adj, features, labels, idx_train, idx_val, idx_test,nx.from_dict_of_lists(graph)
...
def split_dataset(num_nodes, labels, percent, flag_cuda):
    # if public == 1:
    #     if dataset == 'cora':
    ...
    all_data, all_class = np.arange(num_nodes).astype(int), np.arange(labels.shape[1])
    # all_data, all_class = np.arange(num_nodes).astype(int), np.unique(labels.cpu().numpy())
    idx_train, idx_val, idx_test = [], [], []
    for c in all_class:
        # dd = list(np.where(labels.cpu().numpy() == c)[0].astype(int))
        # ee = math.ceil(np.where(labels.cpu().numpy()==c)[0].shape[0]*percent)
        idx_train = np.hstack([idx_train,random.sample(list(np.where(labels[:,c]==1)[0].astype(int)), math.ceil(np.where(labels[:,c]==1)[0].shape[0]*percent))])
    others = np.delete(all_data.astype(int), idx_train.astype(int))
    for c in all_class:
        sss = labels[others]
        idx_val = np.hstack([idx_val,random.sample(list(np.where(labels[others][:,c]==1)[0].astype(int)), math.ceil(500/all_class.shape[0]) )])
    others = np.delete(others.astype(int), idx_val.astype(int))
    for c in all_class:
        idx_test = np.hstack([idx_test,random.sample(list(np.where(labels[others][:,c]==1)[0].astype(int)), min(math.ceil(1000/all_class.shape[0]), np.where(labels[others][:,c]==1)[0].astype(int).shape[0]))])
