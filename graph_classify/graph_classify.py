...
class GCNLayer(nn.Module):
    ...
    def forward(self, feat, adj):
        ...
        for n in range(1, 4):
            ...
            k = torch.bmm(adj, k) #+ self.k_numb1 * k
            D = torch.sum(k / (kappa.unsqueeze(1) * 1), dim=-1)
            # D = (D == 0) * 1 + D
            rrr = torch.bmm(D.unsqueeze(-1), kappa.unsqueeze(1))
            k = (k / rrr) + (1 * k) #+ self.kk * k0
...
class Model(nn.Module):
    ...
    def embed(self, feat, adj, mask):
        __, gv1 = self.forward(adj, feat, mask)
        return (gv1).detach()
...
def train(dataset, gpu, num_layer=4, epoch=40):
    nb_epochs = epoch
    # batch_size = batch
    patience = 20
    lr = 0.001
    ...
    for epoch in range(nb_epochs):
        ...
        loss1 = local_global_loss_(lv1, gv1, 'JSD')
    ...
    embeds = model.embed(features, adj, num_nodes)
        ...
        for l in layers:
            ...
            for e in epoch:
                for i in range(5):
                    ...
                    print(f'Dataset: {d}, Layer:{l}, Epoch: {e}, Seed: {seed}')
                    train(d, gpu, l, e)
        print('################################################')